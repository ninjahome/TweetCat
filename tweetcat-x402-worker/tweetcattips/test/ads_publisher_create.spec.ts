...
describe('Ads Publisher Create Tests', () => {
	beforeEach(async () => {
		if (!routesRegistered) {
			registerSrv(app);
		...
		await env.DB.prepare(
			...
			.bind('a_x_1', '10000000', '0')
			.run();
	});

	it('P-01: Should create ad and reserve budget and bump feed version', async () => {
		const reqBody = {
			a_x_id: 'a_x_1',
			name: 'campaign_1',
			unit_price_atomic: '2000000',
			quota_total: 3,
		...
		expect(typeof data.ad_id).toBe('string');
		expect(data.required_atomic).toBe('6000000');

		const ad = await env.DB.prepare('SELECT ad_id, a_x_id FROM ad_campaigns WHERE ad_id = ?')
			.bind(data.ad_id)
			.first<{ ad_id: string; a_x_id: string }>();
		expect(ad?.ad_id).toBe(data.ad_id);
		expect(ad?.a_x_id).toBe('a_x_1');
		...
		expect(balance?.available_atomic).toBe('4000000');
		expect(balance?.frozen_atomic).toBe('6000000');
