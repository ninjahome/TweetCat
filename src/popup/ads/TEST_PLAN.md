...
| # | 测试场景 | 预期结果 | 优先级 |
|---|---------|---------|--------|
| O-01 | 👋 退出登录 401 错误排查 | 上次遇到的 401 退出错误在本地测试中未出现，需排查生产与本地环境差异 | P1 |
| O-02 | 👋 发布广告的头像异常 | 发布广告的头像只在一个账号中出现了不正确的情况，需排查该特定账号数据 | P1 |
| O-03 | 👋 网络切换时广场刷新 | 当切换测试网、主网后，测试广场(Ad Plaza)应跟随其自动刷新数据 | P1 |
...
| P-13 | ✅ 👋 正常导入：用户通过 x402 导入私信获得的广告数据 | 数据入库成功 | P1 |
| P-14 | ✅ 👋 导入重复数据 | 系统拦截或静默合并，不产生脏数据 | P2 |
| P-15 | ❌ 👋 **性能问题**：广告列表数据加载缓慢 | 数据获取与渲染应流畅（后端查询优化或前端分页优化） | P1 |
...
| E-16 | 👋 **API 频率限制 (429)** | 访问推特 API 触发 429 → 捕捉错误并触发 TokenFreeze → 后面 4 分钟内暂停自动同步任务 | P1 |
...
| # | 测试场景 | 预期结果 | 优先级 |
|---|---------|---------|--------|
| AI-01 | 🤖 **会话健壮性守卫**：CDP 授权过期 (401) | 页面初始化失败时（getCurrentXId 无法执行）增加 guard 拦截，防止抛出 Uncaught Error 崩溃，改为在各入口安全返回并弹出登录提示 | P0 |
