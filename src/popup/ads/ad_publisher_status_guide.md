...
- **派生状态（运行时计算）**：`EXPIRED` / `COMPLETED`（由 `end_date`、`quota_claimed/quota_total` 计算得到；兼容旧数据会取 `max(quota_claimed, quota_used)`）
...
- 完成：任何未 ended 的广告在 `max(quota_claimed, quota_used) >= quota_total` 时派生为 `COMPLETED`

> 注意：当前实现里 **领取会 `quota_claimed + 1`**，但 `quota_used`（已结算/已确认）目前不会增长（因为验证/结算尚未实现）。
...
ACTIVE  ─────────────────>  PAUSED_MANUAL  ────────────────>  ACTIVE
  │
  │ (quota_claimed >= quota_total)  -> COMPLETED   [ended]
...
| `COMPLETED` | 可领取额度用尽（ended，只读） | `quota_claimed >= quota_total`（派生） | 无 | `N/A`（禁用） | 无（只读） | ❌ | ❌ | ❌ |
...
- 领取：`POST /ads/executor/claim`
  - 当前会 `quota_claimed += 1`（占位/限制领取）并创建 claim；不会增加 `quota_used`（不代表已结算消耗）
...
### P0（优先级最高）：修正“领取=消耗”的口径错误（✅ 已完成第一步）

- ✅ 已落地：`/ads/executor/claim` 不再增加 `quota_used`；改为增加 `quota_claimed` 作为“领取占位”。
- 增加可解释口径：
  - `claimed_count`（领取数）
