...
import { getPrivateKeyFromEOA, transferUSDCByX402 } from "../wallet/cdp_wallet";
...
export class AdDeployer {
    ...
    public static async deployAdVault(
        ...
        // 1. 临时获取私钥并构造本地账户
        let privateKey = await getPrivateKeyFromEOA();
        ...
        try {
            // 2. 调用合约预测地址，确保 100% 准确 (避免 TS 侧 Creation Code 差异)
            const predictedAddress = await publicClient.readContract({
                ...
                args: [account.address, platform, usdcAddress, salt]
            } as any) as Address;

            console.log(`[AdDeployer] Predicted AdVault: ${predictedAddress} (Verified by Factory)`);

            // 3. 执行部署操作 (Transaction 1)
            // 调用 AdFactory.deployAd
            ...
            // 5. 如果有初始预算，执行转账 (Transaction 2) - 使用 x402 协议实现无 Gas 转账
            if (initialBudgetAtomic > 0n) {
                const amountStr = (Number(initialBudgetAtomic) / 1_000_000).toString();
                console.log(`[AdDeployer] Sending initial budget: ${amountStr} USDC (Atomic: ${initialBudgetAtomic}) to ${predictedAddress} via x402`);

                const transferHash = await transferUSDCByX402(chainId, predictedAddress, amountStr);
                console.log(`[AdDeployer] Budget Transfer (x402) Hash: ${transferHash}`);
            }

            return deployHash;

        } finally {
            // 6. 立即从内存中抹除私钥敏感信息 (尽可能减少风险窗口)
